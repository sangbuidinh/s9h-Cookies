...
const SYSTEM_TAB_MESSAGE = "Tab hiện tại là trang hệ thống của trình duyệt, không có cookie website để lấy. Hãy mở website thật hoặc chọn chế độ Tên miền / Tất cả cookie.";
...
export function isBrowserInternalUrl(url) {
  return /^(chrome|edge|brave|devtools):\/\//i.test(url || "") || /^about:/i.test(url || "");
}
...
export async function getCurrentTabHostname() {
  ...
  if (isBrowserInternalUrl(tab.url)) {
    throw new Error(SYSTEM_TAB_MESSAGE);
  }
...
async function trySetCookieDetails(details) {
  try {
    ...
    if (details.partitionKey) {
      ...
      return chrome.cookies.set(fallbackDetails);
    }
    throw error;
  }
}
...
export async function setBrowserCookie(cookie) {
  ...
  const activeStoreId = await getCookieStoreIdForActiveTab();
  const storeId = activeStoreId || cookie.storeId;
  if (storeId) {
    details.storeId = storeId;
  }
  ...
  const isSessionCookie = Boolean(cookie.session) ||
    cookie.expirationDate === undefined ||
    cookie.expirationDate === null ||
    expirationDate === 0;
  if (!isSessionCookie && Number.isFinite(expirationDate) && expirationDate > Date.now() / 1000) {
...
export async function removeCookiesForDomain(domain, includeRelated) {
  ...
  const cookiesForStore = storeId ? cookies.filter(function (cookie) {
    return !cookie.storeId || cookie.storeId === storeId;
  }) : cookies;
  let removed = 0;

  for (const cookie of cookiesForStore) {
    try {
      const targetCookie = storeId ? Object.assign({}, cookie, { storeId: storeId }) : cookie;
      const result = await removeBrowserCookie(targetCookie);
      if (result) {
        removed += 1;
      }
    } catch {
